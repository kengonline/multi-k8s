...
kubectl apply -f k8s
kubectl set image deployments/frontend-deployment web=kengonline/k8s-frontend:$SHA
kubectl set image deployments/backend-server-deployment backend-server=kengonline/k8s-backend-server:$SHA
